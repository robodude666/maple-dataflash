...
/** CTOR **/
ATD45DB161D::ATD45DB161D(HardwareSPI *spi) :
...
void ATD45DB161D::ContinuousArrayRead(uint16_t page, uint16_t offset, uint8_t low)
{
	...
	/* Send opcode */
	m_SPI->transfer( low ? AT45DB161D_CONTINUOUS_READ_LOW_FREQ :
...
void ATD45DB161D::BufferRead(uint8_t bufferNum, uint16_t offset, uint8_t low)
{
	...
	if(bufferNum == 1)
	{
		m_SPI->transfer(low ? AT45DB161D_BUFFER_1_READ_LOW_FREQ :
		                   AT45DB161D_BUFFER_1_READ);
	}
	else
	{
		m_SPI->transfer(low ? AT45DB161D_BUFFER_2_READ_LOW_FREQ :
